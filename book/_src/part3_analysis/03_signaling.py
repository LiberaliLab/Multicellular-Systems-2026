...
# %% [markdown]
# Where each was imaged matters, because chapter 02 showed that signal level depends on
...
#
# **PMA is the whole top row.** As chapter 02 showed, it moves everything. It is a real
# effect and it is not a *signaling-specific* effect, so from here on we set it aside and
# look at the other seventeen. It comes back in chapter 08.
#
...
for ax, timepoint in zip(axes, [36, 48, 60, 84]):
    ...
    ax.set(title=f"{timepoint} h", xlabel="log2 p-S6")
    if timepoint == 36:
...
#
# For chapter 08 we need a single summary per condition, so the four themes can be
# compared:
...
# marker-level control SDs from chapter 02 will tell you which.
# :::
...
# ---
#
# **Next:** [04 · Cell-specific mechanics](04_mechanics.ipynb).
